...
export const getCategories = async (req, res) => {
  try {
    ...
    const [totalRows] = await pool.query('SELECT COUNT(*) as total FROM categories WHERE is_active = TRUE')
    const totalCategories = totalRows[0].total

    // Traer solo las categorías activas
    const query = 'SELECT * FROM categories WHERE is_active = TRUE ORDER BY name ASC LIMIT ? OFFSET ?'
...
export const deleteCategory = async (req, res) => {
  try {
    ...
    const [result] = await pool.query('UPDATE categories SET is_active = FALSE WHERE id = ?', [id])
