...
export const createProduct = async (req, res) => {
  ...
  try {
    const [result] = await pool.query('INSERT INTO products (name, description, stock, price, unit, category_id) VALUES (?, ?, ?, ?, ?, ?)', [name, description, stock, price, unit, category_id || null])
...
export const getProducts = async (req, res) => {
  try {
    ...
    // 1. Contamos SOLO los productos activos
    const [totalRows] = await pool.query('SELECT COUNT(*) as total FROM products WHERE is_active = TRUE')
...
export const updateProduct = async (req, res) => {
  try {
    ...
    const [result] = await pool.query('UPDATE products SET name = ?, description = ?, stock = ?, price = ?, unit = ?, category_id = ? WHERE id = ?', [name, description, stock, price, unit, category_id || null, id])
