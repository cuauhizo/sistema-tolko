  ...
  return yup.object({
    ...
    password: yup.string()
      .when([], {
        ...
        otherwise: schema => schema.min(6, 'Mínimo 6 caracteres').nullable().transform(value => value || null), // Si estamos editando, es opcional
      }),
...
<template>
  <div ref="modalElement" class="modal fade" id="userModal" tabindex="-1" aria-labelledby="userModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <Form ref="veeForm" :key="formKey" @submit="handleSubmit" :validation-schema="schema" :initial-values="user" v-slot="{ errors }">
          <div class="modal-body">
            ...
            <div class="mb-3">
              ...
              <Field type="password" class="form-control" :class="{'is-invalid': errors.password}" id="password" name="password" />
              <ErrorMessage name="password" class="invalid-feedback"/>
              <small v-if="isEditMode" class="form-text text-muted">Dejar en blanco para no cambiar la contraseña.</small>
            </div>
            <div class="mb-3">
              <label for="role" class="form-label">Rol</label>
              <Field as="select" class="form-select" :class="{'is-invalid': errors.role_id}" id="role" name="role_id">
