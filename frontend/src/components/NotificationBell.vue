...
<template>
  <li class="relative flex items-center mr-2" ref="bellRef">
    <button @click.prevent="toggleDropdown" class="relative p-2 text-white hover:text-blue-200 focus:outline-none transition-colors rounded-md hover:bg-blue-500">
      <i class="pi pi-bell text-xl"></i>

      <span v-if="unreadCount > 0" class="absolute top-0 right-0 transform translate-x-1/4 -translate-y-1/4 bg-red-500 text-white text-[10px] font-bold px-1.5 py-0.5 rounded-full border-2 border-blue-600">
        {{ unreadCount }}
      </span>
    </button>

    <div v-show="isOpen" class="absolute right-0 top-full mt-2 w-80 max-h-96 overflow-y-auto bg-white rounded-md shadow-xl py-1 z-50 ring-1 ring-black ring-opacity-5">
      <div v-if="notifications.length === 0" class="px-4 py-4 text-sm text-gray-500 text-center">No tienes notificaciones nuevas.</div>
