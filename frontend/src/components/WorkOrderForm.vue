          ...
          <div class="modal-body">
            ...
            <div class="row">
              <div class="col-md-4 mb-3">
                <label for="assigned_to_ids" class="form-label">Asignar a</label>
                <Field name="assigned_to_ids" v-slot="{ field }">
                  <MultiSelect
                    ...
                    class="w-100"
                    :class="{ 'p-invalid': errors.assigned_to_ids }"
                    display="chip" />
                </Field>
                <ErrorMessage name="assigned_to_ids" class="invalid-feedback d-block" />
              </div>
              <div class="col-md-4 mb-3">
            ...
            <div class="row align-items-end">
              <div class="col-md-8 mb-3">
                ...
                <select class="form-select" id="productSearch" v-model="productSearch">
                  ...
                  <option v-for="product in productsStore.products" :key="product.id" :value="product.id">{{ product.name }} (Stock: {{ product.stock }})</option>
                </select>
              </div>
              <div class="col-md-4 mb-3">
              ...
              <table class="table table-sm table-bordered">
                ...
                <tbody>
                  <tr v-if="order.products.length === 0">
                    <td colspan="3" class="text-center text-muted">Aún no se han añadido productos.</td>
                  </tr>
                  <tr v-for="item in order.products" :key="item.product_id">
                    <td>
                      ...
                      <div v-if="item.quantity_used > getProductStock(item.product_id)" class="invalid-feedback d-block mt-1">
                        Advertencia: Stock insuficiente. Disponible:
                    ...
                    <td>
                      <input
                        ...
                        min="1"
                        :class="{
                          'is-invalid': item.quantity_used > getProductStock(item.product_id),
                        }" />
                    </td>
                    <td>
                      <button type="button" class="btn btn-sm btn-outline-danger" @click="removeProduct(item.product_id)">Quitar</button>
                    </td>
                  </tr>
                </tbody>
              </table>
