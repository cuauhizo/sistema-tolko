  ...
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      ...
      >
        ...
        <template #loading>
          <div class="text-center py-4 text-gray-500"><i class="pi pi-spin pi-spinner mr-2"></i> Cargando datos de tareas...</div>
        </template>
        ...
        <Column field="assigned_to" header="Asignada a" :sortable="true">
          <template #body="{ data }">
            <span class="text-blue-600 font-medium"><i class="pi pi-user mr-1 text-xs"></i> {{ data.assigned_to }}</span>
          </template>
        </Column>
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center overflow-x-hidden overflow-y-auto">
      ...
      <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 z-50 transform transition-all">
        ...
        <div class="flex items-center justify-end px-6 py-4 border-t border-gray-100 bg-gray-50 rounded-b-xl space-x-3">
          ...
          <button @click="confirmDeleteTask" class="px-4 py-2 text-sm font-medium text-white bg-red-600 border border-transparent rounded-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 transition-colors flex items-center">
